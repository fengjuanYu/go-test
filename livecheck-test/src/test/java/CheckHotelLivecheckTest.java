...
@Slf4j
public class CheckHotelLivecheckTest {

    private static final String URL_HOTEL = "https://gemini.derbysoftsec.com/gproduct/redirect.ci?Redirect-URL=http://172.19.229.135:8080/gproduct/product.rpc";
    private static final String URL_OLD = "https://skybound.derbysoftsec.com/gagent/redirect.ci?Redirect-URL=http://internal-local-nuke-go-2126578082.ap-southeast-1.elb.amazonaws.com/gagent/dswitch.rpc";
    private static final String URL_NEW="34.217.218.157:9002";//UAT temporarily
    private static final String DISTRIBUTOR = "AGODA";
    private static final String SUPPLIER = "DH";


    private String obtainParamHotel() {
        JSONObject paramHotelJson = new JSONObject();
        paramHotelJson.put("method", "listHotelCodes");
        paramHotelJson.put("params", Lists.newArrayList(SUPPLIER, DISTRIBUTOR));
        log.info(paramHotelJson.toString());
        return paramHotelJson.toString();
    }

    @Test
    public void traverseHotel() throws IOException {
        String hotelIds="";
        String hotelString = doPost(URL_HOTEL, obtainParamHotel());
        ...
        if (StringUtils.isNotEmpty(hotelIds)) {
            ...
            for (Object o : hotelArray) {
                String oldResponse = obtainOldArchitectureData(o.toString());
                ...
                if (!equalFlag){
                    fail("---inconsistent results----supplier:"+SUPPLIER+"---distributor:"+DISTRIBUTOR+"---hotelId:"+o);
                }
        ...
        fail("----------------no active hotelIds from"+SUPPLIER+"----"+DISTRIBUTOR+"----------connectivity");
    }

    public String obtainOldArchitectureData(String hotelId) throws UnsupportedEncodingException {
        JSONObject paramJson = new JSONObject();
        paramJson.put("endpoint", "ccs!/singapore?topic=aws_router_endpoints");
        paramJson.put("sourceId", SUPPLIER);
        paramJson.put("distributorId", DISTRIBUTOR);
        ...
        return doPost(URL_OLD, jsonObject.toString());
    ...
    public List<AvailRoomStayDTO> obtainNewArchitecture(String hotelId) {
        ...
        DefaultHotelBuyerRemoteService remoteService = new DefaultHotelBuyerRemoteService(URL_NEW);
